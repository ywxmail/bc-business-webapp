...
bs.ownershipSelectDialog = {
	...
	clickOk : function() {
		...
		if($grid.hasClass("singleSelect")){//单选
			...
			data.status = $trs.find("td:eq(0)").attr("data-value");
			data.number = $trs.find("td:eq(1)").attr("data-value");
			...
			$tds.each(function(i){
				data.push($.extend({
					...
					number:$($trs.get(i)).find("td:eq(1)").attr("data-value"),
