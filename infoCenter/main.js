...
bs.infoCenter = {
	...
	init : function(option, readonly) {
		...
		$page.find("ul#results").delegate(
				"li:not(.empty)",
				{
					...
					click : function() {
						...
						bs.infoCenter.showCar
								.call($this, $this.attr("data-id"));
					}
				});
		...
		// 车辆信息、联系信息：点击折叠展开
		$page.find(".toggleShow").click(
				function() {
					var $this = $(this).toggleClass(
	...
	findCars : function($left, url, data) {
		...
		$left
				.find("#results")
				.html(
						'<li class="empty busy"><span class="busy"></span><span class="text">正在查询...</span></li>');
		...
		bc
				.ajax({
					...
					success : function(cars) {
						$left.find("#count").html(
								" (" + cars.length
										+ "台)<span class='wasteTime'> "
										+ bc.getWasteTime(startTime)
										+ "</span>");
							...
							lis
									.push('<li data-id="'
											...
											+ (cars[i].status == 1 ? ' - 此车已注销" class="ui-state-disabled"'
													: '"') + ' data-json=\''
											+ $.toJSON(cars[i]) + '\''
											+ '>[<span class="code ellipsis">'
											+ cars[i].code
											+ '</span>]<span class="plate">'
											+ cars[i].plate + '</span></li>');
						}
	...
	doSearch : function() {
		...
		logger.info("bs.infoCenter.doSearch:searchType=" + searchType
		...
		// 执行查询
		bs.infoCenter.findCars($left, bc.root
				+ "/bc-business/infoCenter/findCarsBySearchText", {
	...
	doSearchByUnitAndMotorcade : function() {
		...
		// 执行查询
		bs.infoCenter.findCars($left, bc.root
				+ "/bc-business/infoCenter/findCarsByUnitAndMotorcade", {
			unitId : unitId,
	...
	showCar : function(carId) {
		if (carId == bs.infoCenter.currentCarId)
			return;
		logger.info("carId=" + carId + ",preCarId="
				+ bs.infoCenter.currentCarId);
		...
		var $carTitle = $carHeader.find("#carTitle").html(
				'正在查询' + lj.plate + '...');
		...
		bc
				.ajax({
					...
					success : function(json) {
						...
						$carTitle
								.text(lj.plate
										+ ($li.hasClass("ui-state-disabled") ? " - 此车已注销，交车日期为 "
												+ lj.returnDate
												: ""));
						$carTitle.next().text(
								" (" + bc.getWasteTime(startTime) + ")");
						...
						for ( var key in json.car) {
							$main.find(":input[name='main." + key + "']").val(
									json.car[key]);
						}
						if (json.car.includeCost)
							$main.find(":input[name='main.includeCost']")[0].checked = true;
						if (json.car.zb)
							$main.find(":input[name='main.zb']")[0].checked = true;
						...
						if (msgs.length > 0) {
							...
							for ( var i = 0; i < msgs.length; i++) {
								trs
										.push('<tr class="ui-widget-content middle row'
												+ (i % 2 == 0 ? " odd"
														: " even")
												+ '" data-plate="'
												+ lj.plate
												...
												+ msgs[i].id
												+ '">'
												+ '<td class="first ui-widget-content">'
												+ msgs[i].module
												+ '</td>'
												+ '<td class="middle ui-widget-content">'
												+ msgs[i].limit
												+ '</td>'
												+ '<td class="middle ui-widget-content">'
												+ msgs[i].link
												...
												+ msgs[i].subject
												+ '</td>'
												+ '<td class="last ui-widget-content">'
												+ msgs[i].date
												+ '</td>'
												+ '</tr>');
							}
						...
						if (mans.length > 0) {
							...
							for ( var i = 0; i < mans.length; i++) {
								...
								// 抬头行
								trs
										.push('<tr class="top header'
												+ (i == 0 ? " first" : "")
												+ (man.status != 0 ? " ui-state-disabled"
														: "")
												+ '">'
												+ '<td class="first aright ui-widget-content" style="width: 90px">'
												+ man.type
												+ ':</td>'
												+ '<td class="middle aleft ui-widget-content" style="width: 80px">'
												+ man.name
												+ '('
												+ man.sex
												+ ')'
												...
												+ '<td class="last aleft ui-widget-content" style="width: 190px">'
												+ man.phones
												+ '</td>'
												+ '<td class="middle aright ui-widget-content" style="width: 40px">状态:</td>'
												+ '<td class="last aleft ui-widget-content">'
												+ (man.classes.length > 0 ? man.classes
														+ "，"
														: "") + man.moveType
												+ '(' + man.moveDate + ')'
												+ '</td>' + '</tr>');
								...
								// 司机详细资料
								trs
										.push('<tr class="detail"><td colspan="6" style="padding:0;">'
												+ '<table class="contentTable" cellspacing="2" cellpadding="0" style="height: auto;">'
												+ '<tr>'
												+ '<td rowspan="6" style="width: 87px;"><img style="width:86.4px;height:110px;cursor: pointer;" data-id="'
												+ man.id
												+ '" data-name="'
												+ man.name
												+ '"'
												+ 'src="'
												+ bc.root
												+ '/bc/image/download?ptype=portrait&puid='
												+ man.uid
												+ '&ts='
												+ ts
												+ '"></td>'
												...
												+ '<td class="value" style="width: 250px;"><input type="text" class="ui-widget-content" readonly="readonly" value="'
												+ man.identity
												+ '"/></td>'

												+ '<td rowspan="6" style="position: relative;"><textarea class="ui-widget-content noresize"'
												+ 'style="position:absolute;left:0;right:0;bottom:-2px;top:-2px;" readonly="readonly">'
												+ man.desc
												+ '</textarea></td>'
												+ '</tr>'
												...
												+ '<td class="value"><input type="text" class="ui-widget-content" readonly="readonly" value="'
												+ man.address1
												+ '"/></td>'
												+ '</tr>'
												...
												+ '<td class="value"><input type="text" class="ui-widget-content" readonly="readonly" style="width:86px" value="'
												+ man.cert4fwzg
												+ '"/>'
												+ '&nbsp;&nbsp;户口性质:&nbsp;<input type="text" class="ui-widget-content" readonly="readonly" style="width:86px" value="'
												+ man.houseType
												+ '"/></td>'
												+ '</tr>'
												+ '</table></td></tr>');
